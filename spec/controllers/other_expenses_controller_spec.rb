...
RSpec.describe OtherExpensesController, :type => :controller do

  describe "index" do
    it "render index page" do  
      user = FactoryGirl.create(:user)  
      expense = FactoryGirl.create(:expense, date: (Time.current).to_date, detail: "Lunch", amount: 80, credit: false, user: user, other: true) 
  ...
  describe "create" do
    it "create new other expense" do
      user = FactoryGirl.create(:user) 
      expense = FactoryGirl.create(:expense, date: (Time.current).to_date, detail: "Lunch", amount: 80, credit: false, user: user, other: true) 
      expense_2 = FactoryGirl.create(:expense, date: (Time.current).to_date, detail: "Lunch", amount: 80, credit: false, user: user) #noise 
      date = Time.current  
      ...
      sign_in_as(user) do
        ...
        expect(user.expenses.other_expenses.count).to eq(2)
        ...
        expect(user.expenses.last.other).to eq(true)
  ...
  describe "update" do
    it "update expense" do
      user = FactoryGirl.create(:user)    
      expense = FactoryGirl.create(:expense, date: (Time.current - 3.days).to_date, detail: "Lunch", amount: 80, credit: false, user: user, other: true)
      date = Time.current
      ...
      sign_in_as(user) do
        ...
        expect(expense.other).to eq(true)
        ...
        expect(response.body).to eq({
          ...
          total_for_today: 0,
          total_of_this_month: user.total_of_this_month(other_expense: true),
    ...
    it "update expense and update total money of this month" do
      user = FactoryGirl.create(:user)    
      expense = FactoryGirl.create(:expense, date: Time.current.to_date, detail: "Lunch", amount: 80, credit: false, user: user, other: true)
      expense_2 = FactoryGirl.create(:expense, date: Time.current.to_date, detail: "Lunch", amount: 80, credit: false, user: user, other: true)
  ...
  describe "delete" do
    it "delete expense" do
      user = FactoryGirl.create(:user)    
      expense = FactoryGirl.create(:expense, date: Time.current.to_date, amount: 80, user: user, other: true)
